...
@info "Uploading test data with prefix: $TEST_PREFIX"
blobname, treename = "$(TEST_PREFIX)_Blob", "$(TEST_PREFIX)_Tree"
weirdnames = string.("$(TEST_PREFIX)_", ["foo/bar/baz", "Δεδομένα", "Δε-δο-μέ/να"])
deletename = "$(TEST_PREFIX)_Blob"
...
try
    ...
    @test isempty(list_datasets_prefix(TEST_PREFIX))
    ...
    datasets = list_datasets_prefix(TEST_PREFIX; auth)
    ...
    for dataset in (blobname, treename, deletename, weirdnames...)
        _delete_test_dataset(auth, dataset)
    end
end
@test isempty(list_datasets_prefix(TEST_PREFIX; auth))
